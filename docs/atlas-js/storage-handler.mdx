...
The `StorageHandler` class provides methods for storage operations that interact exclusively with Atlas Protocol's x/storage module and storage providers.
It is designed for clients that need more granular control; hence there is no uploading queueing, no subscription loading, and no filetree integrations.
This is particularily useful for enterprises that want to index files their own way or make use of many subscriptions simultaneously.
